...
def start_scheduler() -> None:
    ...
    """
    global _daily_task, _weekly_task
    ...
    logger.info("insight_scheduler: started (daily + weekly loops)")


async def stop_scheduler() -> None:
    """Cancel scheduled tasks gracefully on shutdown."""
    for task in (_daily_task, _weekly_task):
...
async def _run_daily_loop() -> None:
    """Sleep 24 h, run aggregate_daily_insights(), repeat."""
    while True:
        await asyncio.sleep(_DAILY_INTERVAL_SECONDS)
        try:
            await aggregate_daily_insights()
        except Exception as exc:
            logger.warning("insight_scheduler: daily job failed error=%s", exc)


async def _run_weekly_loop() -> None:
    """Sleep 7 days, run score_weekly_growth_for_all_users(), repeat."""
    while True:

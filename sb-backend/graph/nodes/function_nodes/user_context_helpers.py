...
This module resolves Supabase UID from payload user_id and fetches profile
context from the auth database tables.
"""
...
async def resolve_supabase_uid_from_payload_user_id(supabase_user_id: int) -> str:
    """
    Resolve Supabase UID from payload user_id.
    ...
    async with auth_db.get_session() as session:
        result = await session.execute(stmt, {"user_id": supabase_user_id})
    ...
    try:
        user = await get_user_by_id(str(supabase_user_id))
    except Exception as e:
        raise ValueError(f"Failed to resolve supabase uid from user_id {supabase_user_id}: {str(e)}") from e

    resolved_supabase_uid = getattr(user, "id", None)
    if not resolved_supabase_uid:
        raise ValueError(f"Failed to resolve supabase uid from user_id {supabase_user_id}: missing id")

    return str(resolved_supabase_uid)


async def fetch_user_detailed_profile(supabase_user_id: int) -> Optional[Dict[str, Any]]:
    """

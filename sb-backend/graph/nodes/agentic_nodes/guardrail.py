...
async def guardrail_node(
    state,
    guardrail_fn: Optional[Callable[[str], str]] = None,
) -> Dict[str, Any]:
    """
    Guardrail step node - verifies LLM response against a set of rules.
    """
    prompt = f"""
...
def call_guardrail_llm(prompt) -> str:
    ...
    logger.info(
        "guardrail_node: calling ollama",
        extra={"url": f"{OLLAMA_BASE_URL}/api/generate", "timeout": OLLAMA_TIMEOUT},
    )

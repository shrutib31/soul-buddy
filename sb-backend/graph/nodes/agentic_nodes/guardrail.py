...
GUARDRAIL_RULES = [
    ...
    "Do not NEVER minimize or invalidate distress in a user",
    "Do not increase shame or guilt in a response to the user",
    ...
    "Do not rush into advice or solutions before validation the user and exploring the situation",
...
def guardrail_node(state):
    ...
    Returns:
        String
            ...
            2. REFINE - response has issues and must go through pipeline again, DO NOT SEND!
    """
    prompt = f"""
...
Candidate Assistant Answer: {state.get('response_draft', '')}
GUARDRAIL_RULES: "{GUARDRAIL_RULES}"

"""
    #implement like convo guardrail engine
    try:
        print("Guardrail checking response...")
        guardrailResponse = call_guardrail_llm(prompt)
        data: Dict[str, Any] = safe_json_loads(guardrailResponse)
        guardrailStatus= str(data.get("status", "REFINE")).upper()
        guardrailFeedback= str(data.get("feedback", "")).strip()
        state["guardrail_status"] = guardrailStatus
        state["guardrail_feedback"] = guardrailFeedback

        #Default to REFINE if LLM output is bad
        if (guardrailStatus != "OK" or guardrailStatus != "REFINE"):
            print("Bad Response. Resorting to Refine")
            state["guardrail_status"] = "REFINE"
            state["guardrail_feedback"] = "Bad response. Need to refine"

    except Exception as e:
        print(f"Error in guardrail node: {e}")
        state["response_draft"] = "It sounds like you're going through a lot. What emotion feels strongest right now?"
...
def guardrail_router(state):
    ...
    print("[router] Guardrail says REFINE → back to exploration_state")
